...
{
	return wrap_call(_mrb, [&]() -> mrb_bool {
		switch (mrb_type(obj)) {
		case MRB_TT_CLASS:
		case MRB_TT_SCLASS:
...
RubyState::define_class_method(RClass *cla, const char *name,
    const mrb_func_t fun, const mrb_aspec aspec)
{
	WRAP_CALL(mrb_define_class_method(_mrb, cla, name, fun, aspec));
}
...
RubyState::get_args(const mrb_args_format format, ...)
{
	va_list ap;
	va_start(ap, format);
	auto out = wrap_call(_mrb, [&]() -> mrb_int {
...
{
	return WRAP_CALL(mrb_ary_new_capa(_mrb, n));
}
...
{
	return WRAP_CALL(mrb_calloc(_mrb, n, size));
}
...
{
	return WRAP_CALL(mrb_check_intern_str(_mrb, str));
}
...
{
	return WRAP_CALL(mrb_class_name(_mrb, klass));
}
...
{
	return WRAP_CALL(mrb_const_defined(_mrb, mod, id));
}
...
{
	return WRAP_CALL(mrb_const_get(_mrb, mod, id));
}

void
RubyState::const_remove(const mrb_value mod, const mrb_sym id)
{
	WRAP_CALL(mrb_const_remove(_mrb, mod, id));
}
...
{
	WRAP_CALL(mrb_const_set(_mrb, mod, id, value));
}

double
RubyState::cstr_to_dbl(const char *s, const mrb_bool badcheck)
{
	return wrap_call(_mrb, [&]() -> double {
...
{
	return wrap_call(_mrb, [&]() {
		const auto str = mrb_str_new_cstr(_mrb, s);
...
{
	return WRAP_CALL(mrb_f_raise(_mrb, val));
}
...
{
	WRAP_CALL(mrb_field_write_barrier(_mrb, b1, b2));
}
...
RubyState::format(const char *format, ...)
{
	va_list ap;
	va_start(ap, format);
	auto value = wrap_call(_mrb, [&]() { return vformat(format, ap); });
	va_end(ap);
	return WRAP_CALL(str_new_cstr(string_cstr(value)));
}
    ...
    ...)
{
	va_list ap;
	va_start(ap, argc);
	auto argv = wrap_call(_mrb, [&]() {
		std::vector<mrb_value> out;
		out.reserve(argc);
		for (mrb_int i = 0; i < argc; ++i) {
			out.push_back(va_arg(ap, mrb_value));
		}
		return out;
	});
	va_end(ap);
	return WRAP_CALL(
	    funcall_argv(val, intern_cstr(name), argc, argv.data()));
}
...
{
	return WRAP_CALL(mrb_generate_code(_mrb, p));
}
...
{
	return WRAP_CALL(mrb_iv_copy(_mrb, dst, src));
}

mrb_bool
RubyState::iv_defined(const mrb_value val, const mrb_sym sym)
{
	return WRAP_CALL(mrb_iv_defined(_mrb, val, sym));
}
...
{
	return WRAP_CALL(mrb_load_file(_mrb, fh));
}

mrb_value
RubyState::load_file_cxt(FILE *fh, mrb_ccontext *cxt)
{
	return WRAP_CALL(mrb_load_file_cxt(_mrb, fh, cxt));
}

mrb_value
RubyState::load_irep(const uint8_t *buf)
{
	return WRAP_CALL(mrb_load_irep(_mrb, buf));
}

mrb_value
RubyState::load_irep_buf(const void *buf, const size_t len)
{
	return WRAP_CALL(mrb_load_irep_buf(_mrb, buf, len));
}
...
RubyState::load_irep_buf_cxt(const void *buf, const size_t len,
    mrb_ccontext *cxt)
{
	return WRAP_CALL(mrb_load_irep_buf_cxt(_mrb, buf, len, cxt));
}

mrb_value
RubyState::load_irep_cxt(const uint8_t *buf, mrb_ccontext *cxt)
{
	return WRAP_CALL(mrb_load_irep_cxt(_mrb, buf, cxt));
}

mrb_value
RubyState::load_irep_file(FILE *fh)
{
	return WRAP_CALL(mrb_load_irep_file(_mrb, fh));
}

mrb_value
RubyState::load_irep_file_cxt(FILE *fh, mrb_ccontext *cxt)
{
	return WRAP_CALL(mrb_load_irep_file_cxt(_mrb, fh, cxt));
}
...
{
	return WRAP_CALL(mrb_malloc(_mrb, n));
}

void *
RubyState::malloc_simple(const size_t n)
{
	return WRAP_CALL(mrb_malloc_simple(_mrb, n));
}

mrb_method_t
RubyState::method_search(RClass *cls, const mrb_sym sym)
{
	return WRAP_CALL(mrb_method_search(_mrb, cls, sym));
}

mrb_method_t
RubyState::method_search_vm(RClass **cls, const mrb_sym sym)
{
	return WRAP_CALL(mrb_method_search_vm(_mrb, cls, sym));
}
...
RubyState::name_error(const mrb_sym id, const char *fmt, ...)
{
	...
	va_end(ap);
	const auto mrb = _mrb;
	const auto exc = wrap_call(_mrb, [&]() {
		const auto out = exc_new_str(E_NAME_ERROR, msg);
		iv_set(out, EULER_IVSYM(name), mrb_symbol_value(id));
		return out;
	});
	exc_raise(exc);
}
    ...
    const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	const auto msg = WRAP_CALL(vformat(fmt, ap));
	va_end(ap);
	const auto mrb = _mrb;
	const auto exc = wrap_call(_mrb, [&]() {
		const auto out = exc_new_str(E_NOMETHOD_ERROR, msg);
		iv_set(out, EULER_IVSYM(name), mrb_symbol_value(id));
		iv_set(out, EULER_IVSYM(args), args);
		return out;
	});
	exc_raise(exc);
}
...
{
	return WRAP_CALL(mrb_notimplement_m(_mrb, msg));
}

mrb_value
RubyState::num_minus(const mrb_value x, const mrb_value y)
{
	return WRAP_CALL(mrb_num_sub(_mrb, x, y));
}
...
{
	return WRAP_CALL(mrb_num_add(_mrb, x, y));
}

RBasic *
RubyState::obj_alloc(const mrb_vtype vtype, RClass *cls)
{
	return WRAP_CALL(mrb_obj_alloc(_mrb, vtype, cls));
}
...
{
	WRAP_CALL(mrb_parser_parse(ps, cxt));
}

void
RubyState::parser_set_filename(mrb_parser_state *ps, const char *filename)
{
	WRAP_CALL(mrb_parser_set_filename(ps, filename));
}
...
{
	return WRAP_CALL(mrb_proc_new_cfunc(_mrb, fn));
}
...
{
	return WRAP_CALL(mrb_read_irep(_mrb, buf));
}

mrb_irep *
RubyState::read_irep_buf(const void *buf, const size_t len)
{
	return WRAP_CALL(mrb_read_irep_buf(_mrb, buf, len));
}

void *
RubyState::realloc(void *buf, const size_t len)
{
	return WRAP_CALL(mrb_realloc(_mrb, buf, len));
}

void *
RubyState::realloc_simple(void *buf, const size_t len)
{
	return WRAP_CALL(mrb_realloc_simple(_mrb, buf, len));
}
...
{
	WRAP_CALL(mrb_stack_extend(_mrb, i));
}
...
{
	return WRAP_CALL(mrb_str_new(_mrb, p, len));
}

mrb_value
RubyState::str_new_capa(const size_t capa)
{
	return WRAP_CALL(mrb_str_new_capa(_mrb, capa));
}

mrb_value
RubyState::str_new_static(const char *p, const size_t len)
{
	return WRAP_CALL(mrb_str_new_static(_mrb, p, len));
}
...
{
	return WRAP_CALL(mrb_undef_class_method(_mrb, cls, name));
}

void
RubyState::undef_class_method_id(RClass *cls, const mrb_sym name)
{
	return WRAP_CALL(mrb_undef_class_method_id(_mrb, cls, name));
}

void
RubyState::undef_method(RClass *cla, const char *name)
{
	return WRAP_CALL(mrb_undef_method(_mrb, cla, name));
}

void
RubyState::undef_method_id(RClass *cls, const mrb_sym sym)
{
	return WRAP_CALL(mrb_undef_method_id(_mrb, cls, sym));
}
...
RubyState::warn(const char *fmt, ...)
{
	va_list ap;
	va_start(ap, fmt);
	auto msg = WRAP_CALL(vformat(fmt, ap));
	va_end(ap);
	WRAP_CALL(mrb_warn(_mrb, "%s", string_cstr(msg)));
}
...
{
	return WRAP_CALL(mrb_word_boxing_float_value(_mrb, fl));
}

mrb_value
RubyState::word_boxing_int_value(const mrb_int n)
{
	return WRAP_CALL(mrb_boxing_int_value(_mrb, n));
}
...
{
	const mrb_value v = obj_value(str);
...
{
	return WRAP_CALL(mrb_float(ensure_float_type(x)));
}
