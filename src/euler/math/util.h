...
/* TODO: this got duplicated into util/math.h, wrap it here to allow for
 *       handling size_type */
...
unwrap_complex(const util::Reference<util::RubyState> &mrb, mrb_value arg)
{
	...
	const auto real = static_cast<T>(mrb->ensure_float_type(real_value));
	const auto imag = static_cast<T>(mrb->ensure_float_type(imag_value));
